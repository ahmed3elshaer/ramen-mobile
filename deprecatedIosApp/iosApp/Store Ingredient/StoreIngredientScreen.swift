...
struct StoreIngredientsScreen: View {
    ...
    @StateObject private var store: StoreIngredientWrapper = .init()
                            ...
                            .sheet(isPresented: $selectingDuration) {
                                if #available(iOS 16.0, *) {
                                    DurationPicker { duration, unit in
                                        let durationPerDay = 24 * 60 * 60 * 1_000_000_000 * 2
                                        let durationPerWeek = 24 * 60 * 60 * 1_000_000_000 * 2 * 7
                                        let durationPerMonth = 24 * 60 * 60 * 1_000_000_000 * 2 * 7 * 4
                                        var unitValue: Int {
                                            switch unit {
                                            case "Days":
...
struct DurationPicker: View {
    ...
    let onDurationSelected: (Int, String) -> Void
...
struct DuraionPicker_Previews: PreviewProvider {
    static var previews: some View {
        if #available(iOS 16.0, *) {
            DurationPicker(onDurationSelected: { _, _ in
